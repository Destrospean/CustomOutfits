...
using Sims3.UI;
using BeekeepingBoxObject = Sims3.Gameplay.Objects.Insect.BeekeepingBox;
    ...
    public class BeekeeperOutfit
    {
        ...
        public class ResetBeekeeperOutfit : ImmediateInteraction<Sim, GameObject>
        {
            ...
            public class Definition : ImmediateInteractionDefinition<Sim, GameObject, ResetBeekeeperOutfit>
            {
                ...
                public override bool Test(Sim actor, GameObject target, bool isAutonomous, ref GreyedOutTooltipCallback greyedOutTooltipCallback)
                {
                    return (actor == target && Tuning.kShowSimMenu || target as Sim == null && Tuning.kShowObjectMenu) && actor.SimDescription.TeenOrAbove && actor.SimDescription.IsHuman && !actor.SimDescription.IsRobot && !isAutonomous && actor.SimDescription.HasSpecialOutfit(GetBeekeeperOutfitName(actor));
                }
            }
        ...
        public static void DisableBeekeeperOutfit(SimDescription simDescription)
        {
        ...
        public static void EnableBeekeeperOutfit(SimDescription simDescription)
        {
            if (!GetBeekeeperOutfitEnabled(simDescription))
            {
        ...
        public static bool GetBeekeeperOutfitEnabled(SimDescription simDescription)
        {
        ...
        static void OnObjectPlacedInLot(object sender, EventArgs e)
        {
            ...
            {
                AddInteractions(GameObject.GetObject(onObjectPlacedInLotEventArgs.ObjectId) as BeekeepingBoxObject);
            }
        }

        static void OnPreLoad()
        {
            BeekeepingBoxObject.Clean.Singleton = new BeekeepingBox.Clean.DefinitionModified();
            BeekeepingBoxObject.Feed.Singleton = new BeekeepingBox.Feed.DefinitionModified();
            BeekeepingBoxObject.Harvest.Singleton = new BeekeepingBox.Harvest.DefinitionModified();
            BeekeepingBoxObject.SmokeOut.Singleton = new BeekeepingBox.SmokeOut.DefinitionModified();
            Common.CopyTuning(typeof(BeekeepingBoxObject), typeof(BeekeepingBoxObject.Clean.Definition), typeof(BeekeepingBox.Clean.DefinitionModified));
            Common.CopyTuning(typeof(BeekeepingBoxObject), typeof(BeekeepingBoxObject.Feed.Definition), typeof(BeekeepingBox.Feed.DefinitionModified));
            Common.CopyTuning(typeof(BeekeepingBoxObject), typeof(BeekeepingBoxObject.Harvest.Definition), typeof(BeekeepingBox.Harvest.DefinitionModified));
            Common.CopyTuning(typeof(BeekeepingBoxObject), typeof(BeekeepingBoxObject.SmokeOut.Definition), typeof(BeekeepingBox.SmokeOut.DefinitionModified));
        }
        ...
        {
            Array.ForEach(Sims3.Gameplay.Queries.GetObjects<BeekeepingBoxObject>(), AddInteractions);
...
{
    public class BeekeepingBox
    {
        public class Clean : BeekeepingBoxObject.Clean
        {
            public class DefinitionModified : InteractionDefinition<Sim, BeekeepingBoxObject, Clean>
            {
                public override string GetInteractionName(Sim actor, BeekeepingBoxObject target, InteractionObjectPair interaction)
                {
                    return BeekeepingBoxObject.LocalizeString("CleanBox");
                }

                public override bool Test(Sim actor, BeekeepingBoxObject target, bool isAutonomous, ref GreyedOutTooltipCallback greyedOutTooltipCallback)
                {
        ...
        public class Feed : BeekeepingBoxObject.Feed
        {
            public class DefinitionModified : InteractionDefinition<Sim, BeekeepingBoxObject, Feed>
            {
                public override string GetInteractionName(Sim actor, BeekeepingBoxObject target, InteractionObjectPair interaction)
                {
                    return BeekeepingBoxObject.LocalizeString("FeedBees");
                }
        ...
        public class Harvest : BeekeepingBoxObject.Harvest
        {
            public class DefinitionModified : InteractionDefinition<Sim, BeekeepingBoxObject, Harvest>
            {
                public override string GetInteractionName(Sim actor, BeekeepingBoxObject target, InteractionObjectPair interaction)
                {
                    return BeekeepingBoxObject.LocalizeString("HarvestHoney");
                }

                public string NoHoneyStored()
                {
                    return BeekeepingBoxObject.LocalizeString("NoHoneyStored");
                }
            ...
            public override bool Run()
            {
                ...
                {
                    AnimateSim("Harvest");
                    bool honeyStorageLimitMet = Target.mStoredHoneyInfo.Count == BeekeepingBoxObject.kHoneyStorageLimit;
                    while (Target.mStoredHoneyInfo.Count > 0)
                    {
                        BeekeepingBoxObject.HoneyCreationInfo honeyCreationInfo = Target.mStoredHoneyInfo[0];
                    ...
                    {
                        Target.mHoneyProductionAlarm = Target.AddAlarmRepeating(BeekeepingBoxObject.kHoneyProductionTimeInSimHours, TimeUnit.Hours, Target.ProduceHoney, "Produce Honey", AlarmType.AlwaysPersisted);
                    }
                }
                EndCommodityUpdates(baseInteractionFunctionalityDone);
                StandardExit();
        ...
        public class SmokeOut : BeekeepingBoxObject.SmokeOut
        {
            public class DefinitionModified : InteractionDefinition<Sim, BeekeepingBoxObject, SmokeOut>
            {
                public override bool Test(Sim actor, BeekeepingBoxObject target, bool isAutonomous, ref GreyedOutTooltipCallback greyedOutTooltipCallback)
                {
                    return !target.Charred && target.mCurrentAttackTarget == null;
                ...
                public override string GetInteractionName(Sim actor, BeekeepingBoxObject target, InteractionObjectPair interaction)
                {
                    return BeekeepingBoxObject.LocalizeString("SmokeOut");
                }
            }
        ...
        public static bool ChangeSimToBeekeeperOutfit(Sim actor, out bool swimsuitUsed, BeekeepingBoxObject target)
        {
            if (actor.BuffManager.HasTransformBuff() || actor.GetCurrentOutfitCategoryFromOutfitInGameObject() == OutfitCategories.Singed || !BeekeeperOutfit.GetBeekeeperOutfitEnabled(actor.SimDescription))
            {
                swimsuitUsed = false;
                return false;
            }
            ...
            else
            {
                ...
                string specialOutfitKey = BeekeeperOutfit.GetBeekeeperOutfitName(actor);
        ...
        public static bool DoBaseInteractionFunctionality(StateMachineClient stateMachineClient, Sim actor, InteractionInstance instance, out bool outfitChanged, out bool swimwearUsed, BeekeepingBoxObject target)
        {
            ...
            instance.EnterStateMachine("BeekeepingBoxObject", "Enter", "x", "box");
            bool result = true;
            ...
            if (stingOccurred)
            {
                ...
                if (element == null || element.EffectValue >= BuffManager.BuffDictionary[7533959535590961648].EffectValue)
                {
            ...
            else if (attackOccurred)
            {
                ...
                target.mAttackEndingAlarm = actor.AddAlarmRepeating(3, TimeUnit.Seconds, target.CheckForBeeAttackEnding, "BeekeepingBoxObject.CheckForBeeAttackEnding", AlarmType.AlwaysPersisted);
